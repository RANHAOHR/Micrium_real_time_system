...
static int consumerMask = 0;
OS_FLAGS consumerFlag[MAX_CONSUMERS];
...
static void produce();
static int consume(OS_FLAGS flag);
...
static  void  AppTaskStart (void *p_arg)
{
   ...
   (void)p_arg;
    ...
    OSFlagCreate(&readDone,"DoneReading",0,&err);
    ...
    consumerMask = consumerMask * 2 + 1;
    consumerFlag[consumers] = 0x01 << consumers;
    consumers++;
...
static  void  ConsumerTask (void *p_arg)
{
    OS_ERR      err;
    float delay;
    OS_FLAGS flag = *((OS_FLAGS *)p_arg);     /* my flag in the group */

    while (DEF_TRUE) {
        consume(flag);
        delay = ((float)rand()) * 6 / RAND_MAX;
        OSTimeDlyHMSM(0, 0, 0, (int)delay,
                      OS_OPT_TIME_HMSM_STRICT,
                      &err);
        if (err != OS_ERR_NONE)
...
void produce()
{
  ...
  OSSemPend(&full,0,OS_OPT_PEND_BLOCKING,&ts,&err);         /* cannot write into a full buffer */

  OSMutexPend(&access,0,OS_OPT_PEND_BLOCKING,&ts,&err);     /* access shared resource */

  buffer[wrIndex++] = item;                                 /* produce new item */
  if (wrIndex >= BUFFER_SIZE)
	wrIndex = 0;

  if (rdIndex == wrIndex)
    BSP_LED_On(FULL_LED);                                   /* show if buffer is empty */
  BSP_LED_Off(EMPTY_LED);

  OSMutexPost(&access,OS_OPT_POST_NONE,&err);               /* release shared resource */

  for ( int i = 0; i < consumers; i++)
    OSSemPost(&empty,OS_OPT_POST_1 | OS_OPT_POST_NO_SCHED,&err);  /* notify every consumer */
  OSSched();
}

int consume(OS_FLAGS flag)
{
  OS_ERR err;
  CPU_TS ts;
  int item;
  ...
  OSSemPend(&empty,0,OS_OPT_PEND_BLOCKING,&ts,&err);        /* cannotr read from empty buffer */

  OSMutexPend(&access,0,OS_OPT_PEND_BLOCKING,&ts,&err);     /* access shared resource */

  item = buffer[rdIndex];                                   /* consume */
  OSFlagPost(&readDone,flag,OS_OPT_POST_FLAG_SET,&err);

  OSFlagPend(&readDone,consumerMask,0,OS_OPT_PEND_FLAG_SET_ALL + OS_OPT_PEND_NON_BLOCKING,&ts,&err);
  if ((err & OS_ERR_PEND_WOULD_BLOCK) == 0) {               /* if it would NOT block */
    lastReader = 1;                                         /* we are the last one to consume */
    rdIndex++;
    ...
    OSFlagPost(&readDone,consumerMask,OS_OPT_POST_FLAG_CLR,&err); /* clear all flags */
    OSSemPost(&full,OS_OPT_POST_1 | OS_OPT_POST_NO_SCHED,&err);   /* signal that one item has been consumed completely */
  }

  OSMutexPost(&access,OS_OPT_POST_NO_SCHED,&err);           /* release shared resource */
  if (lastReader == 0)
    OSFlagPend(&readDone,consumerMask,0,OS_OPT_PEND_FLAG_SET_ALL + OS_OPT_PEND_BLOCKING,&ts,&err);  /* if not last one, need to wait for other consumers to get this item */
  else
    OSSched();
