...
static  OS_TCB   SignalTCB;

static  CPU_STK  AppTaskStartStk[APP_TASK_START_STK_SIZE];
static  CPU_STK  MasterStk[DEFAULT_TASK_STK_SIZE];
static  CPU_STK  SignalStk[DEFAULT_TASK_STK_SIZE];
...
static char SlaveNames[NUM_SLAVES][16];
OS_MUTEX access;


int  slave_stack[NUM_SLAVES];

OS_SEM submit_all; //to conferm all is ready
...
static  void  AppTaskStart (void *p_arg)
{
   ...
   (void)p_arg;
    ...
    OSSemCreate(&submit_all,"All ready",0,&err);
    ...
    //  OSTaskCreate((OS_TCB   *)&SignalTCB,
    //               (CPU_CHAR   *)"Signal Task",
    //               (OS_TASK_PTR )Signal,
    //               (void       *)0,
    //               (OS_PRIO     )MASTER_PRIO,
    //               (CPU_STK    *)SignalStk,
...
static  void  Slave (void *p_arg)
{
    ...
    while (DEF_TRUE) {
       ...
       delay(50,1);  /* you can play with the timing */
    }
}
...
static  void  Master (void *p_arg)
{
    while (DEF_TRUE) {
      free_a_slave();
      fixeddelay(200);  /* you can play with the timing */
    }
}
...
//
//   for (int i = 0; i < NUM_SLAVES; i++){
//     OSSemPend(&slave_signal[i],0,OS_OPT_PEND_BLOCKING,&ts,&err);
//   }
// //after all posted
//   for (int i = 0; i < NUM_SLAVES; i++){
//      OSSemPost(&submit_all,OS_OPT_POST_1 | OS_OPT_POST_NO_SCHED,&err);
//      printf("submit_all post counter is  %d\n", submit_all.Ctr);
//   }
// }
/* XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX */
...
/* XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX */
int newest; //to share
void free_a_slave()  //master
{
    CPU_TS ts;
    OS_ERR err;

    /* PUT YOUR SYNC CODE HERE */
    OSSemPend(&submit_all,0,OS_OPT_PEND_BLOCKING, &ts, &err); //if all slave is stacked
    OSTaskSemPost(&Slave[newest -1],OS_OPT_POST_1 | OS_OPT_POST_NO_SCHED,&err );

}

void submit_to_master(int taskno, int *infoPtr) ///slave
{
    CPU_TS ts;
    OS_ERR err;
    ...
    *infoPtr = newest; //the previous
    newest = taskno;
    OSSemPost(&submit_all,OS_OPT_POST_1 | OS_OPT_POST_NO_SCHED,&err);

    OSTaskSemPend(0,OS_OPT_PEND_BLOCKING,&ts,&err); //current task
    newest = *infoPtr;  //give the previous

    OSMutexPost(&access,OS_OPT_POST_NO_SCHED,&err);
}
